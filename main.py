...
# === CONFIG ===
BOT_TOKEN = os.environ.get("7620053279:AAGCR8skkDQt5rRvVMZVNSQtwvQeZVJRt6U")  # Use environment variable for safety!
...
class DB:
    ...
    async def update(self, uid, **f):
        await self.db.execute(
            f"UPDATE users SET {','.join([k+'=?' for k in f])} WHERE user_id=?",
            list(f.values()) + [uid]
        )
        await self.db.commit()
    ...
    async def pop_match(self, uid):
        ...
        cur = await self.db.execute("SELECT user_id FROM queue WHERE user_id!=?", (uid,))
    ...
    async def end_chat(self, uid):
        ...
        if p:
            await self.db.execute("DELETE FROM chats WHERE user_id IN (?, ?)", (uid, p))
        else:
            await self.db.execute("DELETE FROM chats WHERE user_id=?", (uid,))
        await self.db.commit()
        return p
...
async def age(m: types.Message, state: FSMContext):
    if not m.text.isdigit():
        return await m.reply("Send number")
...
async def report(m: types.Message):
    p = await db.get_partner(m.from_user.id)
    if not p:
        return await m.reply("Not chatting.")
